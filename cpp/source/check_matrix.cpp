...
namespace fst
{
    Check_Matrix::Check_Matrix(std::vector<Pauli> in_paulis)
        : number_qubits(in_paulis.size())
        , paulis(std::move(in_paulis))
    {
        for (auto &pauli : paulis)
        {
            if (integral_log_2(pauli.get_x_vector()) == -1)
            {
    ...
    void Check_Matrix::set_linear_and_quadratic_forms(Stabiliser_State &state) const
    {
        ...
        {
            const std::size_t v_j = state.basis_vectors[j];
            const Pauli &p_j = *x_stabilisers[j];
            const std::size_t beta_j = p_j.get_z_vector();
            const bool imag_bit = p_j.get_imag_bit();

            imaginary_part |= integral_pow_2(j) * imag_bit;
            real_linear_part |=
                integral_pow_2(j) * (static_cast<int>(p_j.get_sign_bit()) ^ f2_dot_product(beta_j, v_j ^ state.shift));

            for (std::size_t i = 0; i < j; i++)
            {
                // TODO we are accessing the imag_bits alot, optimise?
                const std::size_t v_i = state.basis_vectors[i];
                const bool other_imag_bit = x_stabilisers[i]->get_imag_bit();
    ...
    {
        state.basis_vectors.clear();
        state.basis_vectors.reserve(state.dim);

        for (const auto &pauli : x_stabilisers)
        {
            state.basis_vectors.push_back(pauli->get_x_vector());
        }
    }
    ...
    void Check_Matrix::set_shift(fst::Stabiliser_State &state) const
    {
        ...
        {
            std::size_t pivot_index = integral_log_2(pauli->get_z_vector());
            shift |= (integral_pow_2(pivot_index) * pauli->get_sign_bit());
        }
    ...
    void Check_Matrix::row_reduce_x_stabilisers()
    {
        for (std::size_t i = 0; i < x_stabilisers.size(); i++)
        {
            ...
            const int pivot_index = integral_log_2(pauli.get_x_vector());
            ...
            {
                const auto u_pivot_index = static_cast<std::size_t>(pivot_index);
                for (Pauli *other_pauli : x_stabilisers)
                {
                    if (other_pauli != &pauli && bit_set_at(other_pauli->get_x_vector(), u_pivot_index))
                    {
    ...
    {
        for (Pauli *pauli : z_only_stabilisers)
        {
            const std::size_t pivot_index = integral_log_2(pauli->get_z_vector());

            for (Pauli *other_pauli : z_only_stabilisers)
            {
                if (other_pauli != pauli && bit_set_at(other_pauli->get_z_vector(), pivot_index))
                {
                    (*other_pauli).multiply_by_pauli_on_right(*pauli);
                }
            }
