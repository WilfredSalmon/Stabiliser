...
namespace
{
        ...
        -> std::conditional_t<return_state, std::optional<fst::Stabiliser_State>, bool>
    {
        ...
        // Our reserve should always be correct capacity to avoid re-allocations
        assert(support_size <= indices_capacity);
        ...
        if constexpr (!assume_valid)
        {
            ...
            for (std::size_t i = 1; i < support_size; i++)
            {
                ...
                const std::complex<float> quadratic_eval =
                    evaluate_quadratic_form(new_vector_index, quadratic_form_span);
...
fst::Stabiliser_State fst::stabiliser_from_statevector(const std::span<const std::complex<float>> statevector,
                                                       bool assume_valid)
{
